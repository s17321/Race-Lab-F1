...
def extractRacePace(fp2_session, driver):
    laps = fp2_session.laps.pick_driver(driver)
...
def extractFastestLap(fp3_session, driver):
    laps = fp3_session.laps.pick_driver(driver)
    if not laps.empty:
...
def get_session_data(year, grand_prix, session_name):
    try:
        ...
        print(f"Session type '{session_name}' does not exist for {grand_prix} {year}. Skipping this event.")
        return None
...
def collect_basic_data_for_season(year, grand_prix):
    ...
    if not race_session or not fp2_session or not quali_session:
...
if __name__ == '__main__':
    ...
    # rounds_to_process = event_schedule.head(10)  # Get the first 12 rounds

    # event_schedule = ff1.get_event_schedule(year)
    rounds_to_process = event_schedule.iloc[16:23]  # Get rounds from 12 to 20

    for grand_prix in rounds_to_process['EventName']:
        if 'test' not in grand_prix.lower():  # Skip pre-season tests
    ...
    all_data.to_csv('data/f1_basic_data_2017_first_16-23_rounds.csv', index=False)
