...
# Permutacja cech
def permutation_importance_nn(model, X_test, y_test, metric, n_repeats=10, random_state=42):
    rng = np.random.RandomState(random_state)
    baseline_score = metric(y_test, (model(X_test) > 0.5).float())
    scores = np.zeros((X_test.shape[1], n_repeats))

    for i in range(X_test.shape[1]):
        X_test_permuted = X_test.clone()
        for n in range(n_repeats):
            rng.shuffle(X_test_permuted[:, i].numpy())
            permuted_score = metric(y_test, (model(X_test_permuted) > 0.5).float())
...
# Obliczenie ważności cech dla top 10
importances_top10 = permutation_importance_nn(model_top10, X_test_tensor, y_test_tensor_top10, accuracy_metric)

# Obliczenie ważności cech dla top 3
importances_top3 = permutation_importance_nn(model_top3, X_test_tensor, y_test_tensor_top3, accuracy_metric)

# Wyświetlenie ważności cech dla top 10
feature_names = features.columns
importance_df_top10 = pd.DataFrame({'Feature': feature_names, 'Importance': importances_top10})
importance_df_top10 = importance_df_top10.sort_values(by='Importance', ascending=False)

print("Feature Importances for Top 10:")
print(importance_df_top10)

# Wyświetlenie ważności cech dla top 3
importance_df_top3 = pd.DataFrame({'Feature': feature_names, 'Importance': importances_top3})
importance_df_top3 = importance_df_top3.sort_values(by='Importance', ascending=False)

print("Feature Importances for Top 3:")
print(importance_df_top3)

# Wykres ważności cech dla top 10
plt.figure(figsize=(10, 6))
plt.barh(importance_df_top10['Feature'], importance_df_top10['Importance'], color='skyblue')
plt.xlabel('Importance')
plt.ylabel('Feature')
plt.title('Feature Importances for Predicting Top 10')
plt.gca().invert_yaxis()
plt.show()

# Wykres ważności cech dla top 3
plt.figure(figsize=(10, 6))
plt.barh(importance_df_top3['Feature'], importance_df_top3['Importance'], color='skyblue')
plt.xlabel('Importance')
plt.ylabel('Feature')
plt.title('Feature Importances for Predicting Top 3')
plt.gca().invert_yaxis()
plt.show()
