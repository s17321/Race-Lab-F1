...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output_path', type=str, default='output', help='Ścieżka do katalogu, gdzie zostaną zapisane wyniki (domyślnie: output)')
...
def train_network(initial_learning_rate: float, epochs: int,
    ...
    # Save the model
    torch.save(model.state_dict(), os.path.join(models_dir, f'{task}_model.pth'))
    
    # Save the training history
    history_df = pd.DataFrame(history)
    history_df.to_csv(os.path.join(output_path, f'{task}_training_history.csv'), index=False)
...
def main(args):
    ...
    print(f"Top 10 - Accuracy: {accuracy_points}, Precision: {precision_points}, Recall: {recall_points}, F1 Score: {f1_points}")

    accuracy_podium, precision_podium, recall_podium, f1_podium = evaluate_model(model_podium, val_loader_podium, device)
    print(f"Top 3 - Accuracy: {accuracy_podium}, Precision: {precision_podium}, Recall: {recall_podium}, F1 Score: {f1_podium}")
